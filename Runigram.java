...
public class Runigram {
	...
	public static void main(String[] args) {
		...
		imageOut = scaled(tinypic, 5, 10);
		print(imageOut);

		Color a = new Color(200, 30, 150);
	...
	public static Color[][] flippedHorizontally(Color[][] image) 
	{
		...
		for (int i = 0; i < FlippedImage.length ; i++ ) 
		{
			...
			{
				//This calculation is the key to achieving the horizontal flip
	...
	public static Color[][] scaled(Color[][] image, int width, int height) 
	{
		...
		double scale_height = (double)image.length / height; 
		double scale_width = (double)image[0].length / width;
		for (int i = 0; i < height ; i++ ) 
		{
			for (int j = 0 ; j < width ; j++ ) 
			{
				int height_rescale = (int)(i * scale_height);
				int width_rescale = (int)(j * scale_width);
				Scaled[i][j] = image[height_rescale][width_rescale];
			}
		}
		return Scaled;
	}
